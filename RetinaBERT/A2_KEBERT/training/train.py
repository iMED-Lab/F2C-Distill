...
def get_tokenizer(text,tokenizer,max_length):
    token_list = tokenizer(list(text),add_special_tokens=True,max_length=max_length,padding =True,return_tensors='pt')
    # token_list = tokenizer(list(text), add_special_tokens=True, max_length=max_length, padding="max_length", return_tensors='pt')
...
def train_one_epoch(model,tokenizer, dataloader,num_batches,num_samples, epoch, optimizer, scaler, scheduler, args, tb_writer=None):
    ...
    vis = Visualizer(env="BERT_Training_Loss", port=9000)
    ...
    for i, batch in enumerate(dataloader):
        ...
        with torch.amp.autocast(device_type=device.type, enabled=args.precision == 'amp'):
        ...
        if is_master(args) and (i % 100 == 0 or batch_count == num_batches_per_epoch):
            ...
            samples_per_epoch = num_batches_per_epoch * batch_size * args.world_size  # 确保计算正确
            ...
            if is_master(args) and (i % 100 == 0 or batch_count == num_batches_per_epoch):
                ...
                vis.plot(win="train_loss", y=loss_m.val, con_point=step,
                         opts=dict(title="Train_Loss", xlabel="Step", ylabel="Loss"))
