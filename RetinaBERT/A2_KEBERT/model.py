...
class CLP_clinical(nn.Module):
    ...
    def _get_bert_basemodel(self, bert_model_name, freeze_layers=None):#12
        try:
            # if bert_model_name.endswith('.pt') or bert_model_name.endswith('.pth'):
                # 如果是本地 checkpoint.pt 文件，手动加载
                # print(f"Loading model from checkpoint: {bert_model_name}")
                # config = BertConfig.from_pretrained("./BiomedNLP", output_hidden_states=True)  # 需匹配你的BERT架构
                # model = AutoModel.from_config(config)  # 先用 config 初始化模型
                # checkpoint = torch.load(bert_model_name, map_location=torch.device("cpu"))  # 加载 checkpoint
