...
#         true_label_for_class = y_true[:, i]
#         pred_label_for_class = y_pred[:, i]
#
#         # 计算这个类别的混淆矩阵
#         cm = confusion_matrix(true_label_for_class, pred_label_for_class)
#
#         if cm.shape == (2, 2):
...
#
#     # 计算平均指标
#     average_sen = np.mean(sensitivities)
#     average_spe = np.mean(specificities)
#     average_f1_ss = np.mean(f1_scores)
#
#     # 将所有指标返回，包括每个类别的以及平均值
#     return np.array(sensitivities), np.array(specificities), np.array(
#         f1_scores), average_sen, average_spe, average_f1_ss
